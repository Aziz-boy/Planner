...
catch { console.warn('PDF export is unavailable until pdfkit is installed.'); }
try { mammoth = require('mammoth'); }
catch { console.warn('Word import is unavailable until mammoth is installed.'); }
const config      = require('./config');
const firebaseStore = require('./services/firebase-store');
const pushService = require('./services/push-service');
const {
  openai,
  model: OPENAI_MODEL,
  isOpenAIConfigured,
} = require('./services/openai-client');

const {
  readPlannerData: readFirebaseData,
  setPlannerField,
  writeReflectReport: writeReflectToFirebase,
} = firebaseStore;
const {
  addSubscription,
  loadSubscriptions: loadSubs,
  push,
  removeSubscription,
} = pushService;
...
const bot = telegramEnabled
  ...
  : {
      ...
      getFileLink: async () => { throw new Error('Telegram is not configured.'); },
    };

if (!telegramEnabled) {
  console.warn('Telegram is not configured; web API will continue without polling.');
} else {
...
function tg(text, opts = {}) {
  if (!telegramEnabled) return Promise.resolve(null);
...
function prayerWasCompleted(value) {
  if (value === 'ontime' || value === 'late') return true;
  return Boolean(value && typeof value === 'object' && value.done === true);
}
...
async function generateLawPDF(title, articles, analyses) {
  if (!PDFDocument) throw new Error('PDF export is not installed on the server.');
...
async function getAIRoast() {
  if (!isOpenAIConfigured()) return MOTIVE; // fallback if no OpenAI
  try {
...
cron.schedule('5 9 * * *', async () => {
  ...
  try {
    ...
    const pMissed = pNames.filter(p => !prayerWasCompleted(pYest[p])).join(', ');
...
bot.onText(/\/today/, async () => {
  ...
  try {
    ...
    const tashHour = new Date(Date.now() + 5*3600*1000).getUTCHours();
    const tasksDue = [
...
bot.onText(/\/status/, () => {
  const now = new Date(Date.now() + 5 * 3600 * 1000);
  tg(`✅ Bot running\n🕐 Tashkent: ${now.toISOString().replace('T',' ').slice(0,16)}\n🧠 OpenAI: ${isOpenAIConfigured() ? '✅' : '❌ not set'}\n🔥 Firebase: ${firebaseStore.isFirebaseConfigured() ? '✅' : '❌ not set'}\n📡 Push subscriptions: ${loadSubs().length}`);
});
bot.onText(/\/prayers/, () => {
...
bot.on('message', async (msg) => {
  ...
  if (msg.reply_to_message && pendingFoodCheck.has(msg.reply_to_message.message_id)) {
    ...
    try {
      ...
      const saved = await setPlannerField('diet', diet);
      if (!saved) throw new Error('Firebase is not configured.');
...
bot.on('document', async (msg) => {
  ...
  if (!mammoth) {
    tg('⚠️ Word import is currently unavailable on the server.');
    return;
  }
...
function aiRateLimit(req, res, next) {
  ...
  const recent = (aiRequestBuckets.get(key) || []).filter(timestamp => now - timestamp < windowMs);
...
app.get('/health', (req, res) => res.json({
  ...
  firebase: firebaseStore.isFirebaseConfigured(),
  telegram: telegramEnabled,
}));
app.get('/vapid-public-key', (req, res) => res.json({ key: config.vapid.publicKey || null }));

// Web AI Coach chat endpoint
app.post('/chat', aiRateLimit, async (req, res) => {
  ...
  try {
    ...
    const safeContext = context && typeof context === 'object' ? {
      ...
      completionRate: Number(context.completionRate) || 0,
      prayerStreak: Number(context.prayerStreak) || 0,
      dietCompliance: Number(context.dietCompliance) || 0,
      currentWeight: Number(context.currentWeight) || null,
      savedAmount: Number(context.savedAmount) || 0,
      tasksCompletedToday: Number(context.tasksCompletedToday) || 0,
      tasksTotalToday: Number(context.tasksTotalToday) || 0,
      pendingTasksToday: Array.isArray(context.pendingTasksToday)
        ? context.pendingTasksToday.slice(0, 8).map(item => String(item).slice(0, 160))
        : [],
    } : null;
    const contextPrompt = safeContext
      ? `\n\nCURRENT PLANNER DATA (use it when relevant):\n${JSON.stringify(safeContext)}`
      : '';
    const msgs = [
      { role: 'system', content: (systemPrompts[mode] || systemPrompts.coach) + contextPrompt },
      ...safeHistory,
      { role: 'user', content: message.trim() }
    ];
    const resp = await openai.chat.completions.create({
      model: OPENAI_MODEL, messages: msgs, max_tokens: 250, temperature: 0.7
    });
    const reply = resp.choices?.[0]?.message?.content;
    if (!reply) throw new Error('OpenAI returned an empty response.');
    res.json({ reply });
  } catch(e) {
    console.error('Chat endpoint failed:', e.message);
    res.status(e.statusCode || 500).json({ error: e.message || 'AI request failed.' });
  }
});

// ── AI Plan Generator ─────────────────────────────────────────────────────────
app.post('/generate-plan', aiRateLimit, async (req, res) => {
  const { goals, situation, vision, startDate, progress } = req.body;
  if (typeof goals !== 'string' || !goals.trim()) {
    return res.status(400).json({ error: 'goals required' });
  }
  if ([goals, situation, vision].some(value => typeof value === 'string' && value.length > 8000)) {
    return res.status(400).json({ error: 'plan input is too long' });
  }
  if (!isOpenAIConfigured()) return res.status(503).json({ error: 'OpenAI not configured' });
...
Be deeply specific. Make every task text reference the actual goal for that week.`
    ...
    if (!plan || !Array.isArray(plan.quarters) || typeof plan.vision !== 'object') {
      throw new Error('OpenAI returned an incomplete plan. Please try again.');
    }
...
app.post('/subscribe', (req, res) => {
  const sub = req.body;
  if (!sub || typeof sub.endpoint !== 'string' || !sub.endpoint.startsWith('https://') || sub.endpoint.length > 2048) {
...
app.post('/unsubscribe', (req, res) => {
  ...
  const total = removeSubscription(endpoint);
  res.json({ ok: true, total });
});
...
app.post('/plan-studies', aiRateLimit, async (req, res) => {
  const { rawText, startDate, month, context = {} } = req.body;
  if (typeof rawText !== 'string' || !rawText.trim()) return res.status(400).json({ error: 'rawText required' });
  if (rawText.length > 50000) return res.status(400).json({ error: 'study list is too long' });
  if (!isOpenAIConfigured()) return res.status(503).json({ error: 'OpenAI not configured' });
...
OUTPUT EXACTLY THIS JSON SCHEMA:
{
  ...
  "sessions": [
    ...
    if (!plan || !Array.isArray(plan.sessions)) {
      throw new Error('OpenAI returned an incomplete study plan. Please try again.');
    }
    res.json({ plan });
  } catch(e) {
...
// ── Week AI Summarizer ────────────────────────────────────────────────────────
app.post('/summarize-week', aiRateLimit, async (req, res) => {
  const { days } = req.body; // [{date, done:[taskTexts]}]
  if (!Array.isArray(days) || !days.length || days.length > 7) {
    return res.status(400).json({ error: '1-7 days required' });
  }
  if (days.some(day => !day || typeof day.date !== 'string' || !Array.isArray(day.done))) {
    return res.status(400).json({ error: 'invalid days data' });
  }
  if (!isOpenAIConfigured()) return res.status(503).json({ error: 'OpenAI not configured' });
  try {
...
Output JSON:
{
  ...
  "highlights": ["2-3 specific achievements worth noting"]
    ...
    if (!data || typeof data.summary !== 'string' || !Array.isArray(data.skills)) {
      throw new Error('OpenAI returned an incomplete summary. Please try again.');
    }
    res.json(data);
  } catch(e) {
    res.status(500).json({ error: e.message });
...
app.use((error, req, res, next) => {
  ...
  return res.status(status).json({
    error: status === 400 ? 'Invalid JSON request body.' : 'Unexpected server error.',
  });
});
...
function startServer(port = PORT) {
  return app.listen(port, () => {
    ...
    console.log(`🧠 OpenAI: ${isOpenAIConfigured() ? 'connected' : 'NOT SET'}`);
    console.log(`🔥 Firebase: ${firebaseStore.isFirebaseConfigured() ? 'connected' : 'NOT SET'}`);
